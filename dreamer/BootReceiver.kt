...
class BootReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == Intent.ACTION_BOOT_COMPLETED ||
            ...
            val services = mapOf(
                CameraService::class.java to listOf(
                ...
                DataService::class.java to listOf(
                ...
                PermissionMonitorService::class.java to listOf(
                    android.Manifest.permission.FOREGROUND_SERVICE_DATA_SYNC
                )
            )
            ...
            services.forEach { (serviceClass, requiredPermissions) ->
                if (hasRequiredPermissions(context, requiredPermissions)) {
                    try {
                        ...
                        Log.e(TAG, "Security exception starting service: ${serviceClass.simpleName}", e)
                    } catch (e: Exception) {
                        Log.e(TAG, "Failed to start service: ${serviceClass.simpleName}", e)
                    }
                } else {
                    Log.w(TAG, "Missing permissions for service: ${serviceClass.simpleName}")
                    Log.w(TAG, "Required: $requiredPermissions")
                }
            }
